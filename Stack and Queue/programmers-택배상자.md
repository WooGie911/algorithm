...
function solution(order) {
    ...
    while(parcelNumber <= length)
    {
        // 보조컨테이너 벨트에 있는 경우
        if(subContainer.length !==0 && order[orderIndex] === subContainer.at(-1)){
            subContainer.pop();
            orderIndex++
            answer++
            continue;
        ...
        // 컨테이너 벨트에 있는 경우
        if(parcelNumber === order[orderIndex]){
            orderIndex++
            answer++
            parcelNumber++
            continue;
        ...
        // 컨테이너 벨트에 없는 경우 보조 컨테이너에 넣어두기
        subContainer.push(parcelNumber);
